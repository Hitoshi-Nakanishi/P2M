...
flags.DEFINE_string('data_list', 'pixel2meshpy3tf12/utils/train_list.txt', 'Data list path.')
...
# Train model
train_loss = open('pixel2meshpy3tf12/utils/record_training_loss.log', 'a')
train_loss.write('Start training, lr =  %f\n'%(FLAGS.learning_rate))
train_loss2 = open('pixel2meshpy3tf12/utils/record_training_loss_full.log', 'a')
train_loss2.write('Start training, lr =  %f\n'%(FLAGS.learning_rate))
...
for epoch in tqdm(range(FLAGS.epochs)):
    ...
    for iters in range(train_number):
        ...
        if (iters+1) % 100 == 0:
            ...
            train_loss2.write('%d, %d, %f, %f, %d\n'%(epoch+1,iters + 1, mean_loss, dists,data.queue.qsize()))
            train_loss2.flush()
    model.save(sess)
